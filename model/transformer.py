...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, model_dim: int, num_heads: int):
        ...
        self.mh_attn = self.MultiHeadedSelfAttention(model_dim, num_heads)
    ...
    def forward(self, embedded: TensorType[float]) -> TensorType[float]:
        ...
        embedded_res =  embedded.clone()
        embedded = embedded_res + self.mh_attn(self.ln1(embedded))
        embedded_res = embedded.clone()
        embedded = embedded_res + self.ffn(self.ln2(embedded))
        return embedded
